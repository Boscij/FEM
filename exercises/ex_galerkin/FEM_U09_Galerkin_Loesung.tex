...
\thispagestyle{empty}
\noindent\includegraphics[width=\linewidth]{Logo.png}

\bigskip
\begin{minipage}[t]{0.55\textwidth}
  \textbf{\Huge FE-Grundlagen}\\[.4ex]
  Sebastian Domaschke, mailto: \href{mailto:domh@zhaw.ch}{domh@zhaw.ch}
\end{minipage}
\hfill
...
\]
Wähle gemäss Galerkin Testfunktionen aus dem selben Ansatzraum:
\[
g_u(x)=\delta c_0(x-L)+\delta c_1(x^2-L^2).
\]
Ableitungen:
\[
u'(x)=c_1+2c_2x.
...
\[
u'(x)=c_1+2c_2x,\qquad
g_u'(x)=\delta c_0+2\delta c_1x,\qquad
g_u(0)=-\delta c_0L-\delta c_1L^2.
\]

Einsetzen in \eqref{eqn:Weak} ergibt (nach Kürzen von $A$):
\[
0=-E\!\int_0^L(\delta c_0+2\delta c_1x)(c_1+2c_2x)\,\dd x
 -\,g_u(0)\,\bar t_0
 +\,\nu\!\int_0^L(\delta c_0(x-L)+\delta c_1(x^2-L^2))x^2\,\dd x.
\]

\textbf{Steifigkeitsterm:}
\[
-E\!\int_0^L(\delta c_0+2\delta c_1x)(c_1+2c_2x)\,\dd x
=-E\!\int_0^L\!\Bigl(\delta c_0c_1+2\delta c_0c_2x+2\delta c_1c_1x+4\delta c_1c_2x^2\Bigr)\dd x
\]
\[
=-E\Bigl(\delta c_0c_1L+\delta c_0c_2L^2+\delta c_1c_1L^2+\frac{4}{3}\delta c_1c_2L^3\Bigr).
\]

\textbf{Randterm:}
\[
-g_u(0)\bar t_0=(\delta c_0L+\delta c_1L^2)\bar t_0.
\]

\textbf{Lastterm:}
\[
\nu\!\int_0^L(\delta c_0(x-L)+\delta c_1(x^2-L^2))x^2\,\dd x
=\nu\!\int_0^L\!\Bigl(\delta c_0(x^3-Lx^2)+\delta c_1(x^4-L^2x^2)\Bigr)\dd x
\]
\[
=\nu\Bigl(\delta c_0\bigl(\tfrac{L^4}{4}-\tfrac{L^4}{3}\bigr)
  \delta c_1\bigl(\tfrac{L^5}{5}-\tfrac{L^5}{3}\bigr)\Bigr)
=-\nu\Bigl(\delta c_0\frac{L^4}{12}+\delta c_1\frac{2}{15}L^5\Bigr).
\]

Zusammen:
\[
0=\delta c_0\Bigl(-E(c_1L+c_2L^2)+L\bar t_0-\nu\frac{L^4}{12}\Bigr)
\;+\;\delta c_1\Bigl(-E\!\left(c_1L^2+\frac{4}{3}c_2L^3\right)+L^2\bar t_0-\nu\frac{2}{15}L^5\Bigr).
\]

Da $\delta c_0$ und $\delta c_1$ unabhängig sind, folgen die zwei Gleichungen (jeweils durch $L$ bzw.\ $L^2$ gekürzt):
\[
E(c_1+Lc_2)=\bar t_0-\nu\frac{L^3}{12},\qquad
...
\item[\circled{1}] \textbf{Für welche Gewichtungsfunktionen $g$ erfüllt $\hat u$ die schwache Form?}\\
...
\item[\circled{2}] \textbf{Für welche Gewichtungsfunktionen $g_u$ erfüllen die Näherungen aus Teil I die schwache Form?}\\
...
\item[\circled{3}] \textbf{Durch welche Massnahmen könnte die Näherung verbessert werden?}\\
Ansatzraum erweitern (mehr Freiheitsgrade), oder stückweise Ansatzfunktionen (klassische FEM), also $p$-, $h$- oder $hp$-Verfeinerung.

\item[\circled{4}] \textbf{Wie konkret vorgehen?}\\
Entweder 
\begin{itemize}
